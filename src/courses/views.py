...
def course_detail_view(request, pk):
    if request.method == "GET":
        ...
        course_parts = CoursePart.objects.filter(course_id=pk).prefetch_related(
            Prefetch(
                "topics", queryset=CourseTopic.objects.all(), to_attr="prefetched_topics"
            )
        )
        ...
        return render(
            ...
            {"course": course, "course_parts": course_parts},
        )
...
def course_part_list_view(request):
    ...
    return render(
        request, "courses/coursepart_list.html", {"coursepart_list": courseparts}
    )
...
def course_part_detail_view(request, pk):
    if request.method == "GET":
        ...
        return render(
            request, "courses/coursepart_detail.html", {"coursepart": courseparts}
        )
...
def course_part_create_with_course_view(request, course_id):
    ...
    if request.method == "GET":
        ...
        return render(
            request,
            "courses/coursepart_form_with_course.html",
...
def course_part_delete_view(request, pk):
    ...
    if request.method == "GET":
        return render(
            request, "courses/coursepart_confirm_delete.html", {"coursepart": coursepart}
        )
    elif request.method == "POST":
        coursepart.delete()
        return redirect("coursepart_list")
...
def course_topic_list_view(request):
    ...
    return render(
        request, "courses/coursetopic_list.html", {"coursetopic_list": coursetopics}
    )
...
def course_topic_detail_view(request, pk):
    if request.method == "GET":
        ...
        return render(
            request, "courses/coursetopic_detail.html", {"coursetopic": coursetopic}
        )
...
def course_topic_delete_view(request, pk):
    ...
    if request.method == "GET":
        return render(
            request,
            "courses/coursetopic_confirm_delete.html",
...
def document_list(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            form.save()
    return render(
        request, "document/document_list.html", {"form": form, "documents": docs}
    )
